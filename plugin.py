#           Fronius SmartMeter Plugin from Inverter Plugin
#
#           Author:     ADJ, 2018; pierre42100, 2018
#
"""
<plugin key="froniusSmartMeter" name="Fronius Smart Meter" author="pierre42100" version="0.0.1" wikilink="https://github.com/aukedejong/domoticz-fronius-inverter-plugin.git" externallink="http://www.fronius.com">
    <params>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if (len(Devices) == 0):
            ...
            logDebugMessage("Device created.")
        ...
        Devices[1].Update(0, sValue=str(Devices[1].sValue), Image=Images["FroniusInverterOff"].ID)
    ...
    def onHeartbeat(self):
        ...
        if self.intervalCounter == 1:
            ...
            jsonObject = self.getMeterRealtimeData( ipAddress )

            if (self.isMeterActive(jsonObject)):
    ...
    def getMeterRealtimeData(self, ipAddress):
    ...
    def isMeterActive(self, jsonObject):
    ...
    def updateDeviceCurrent(self, jsonObject):
        ...
        currentWatts = jsonObject["Body"]["Data"]["Site"]["P_Grid"]

        logDebugMessage('Images["PowerWallSocket"].ID = ' + str(Images));
        Devices[1].Update(math.floor(currentWatts), str(currentWatts), Images["FroniusInverterOff"].ID)
...
def logDebugMessage(message):
    if (Parameters["Mode6"] == "Debug"):
        ...
        f = open(Parameters["HomeFolder"] + "fronius-smartmeter-plugin.log", "a")
